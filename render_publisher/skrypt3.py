...
import cv2
project_folder = os.path.expanduser("/home/maciej_br/mgr/synthetic-rgbd-camera-model")
...
class ImagePairPublisher(Node):
    def __init__(self, folder_path: str, scan_period: float = 1.0):
        ...
        self.processor = ImageProcessor(params_path="./femto_mega.json", image_dir="./renders2", output_dir="./output")
        ...
        if not os.path.isdir(self.folder_path):
            self.get_logger().error(f"Podana ścieżka nie istnieje lub nie jest folderem: {self.folder_path}")
            rclpy.shutdown()
            return
        ...
        self.get_logger().info(f"Uruchomiono ImagePairPublisher, skanując folder: {self.folder_path}")
    ...
    def timer_callback(self):
        ...
        rgb_image, depth_image = self.processor.process_single_img_pair_no_save(rgb_path, depth_path)
